...
The master-detail example uses `Split`, but many real interfaces collapse the detail pane into navigation on narrow screens. Optional reference-viewport geometry can faithfully describe either observation, but encoding both without framework syntax or duplicate trees still needs a semantic variant model.

**v0.1 decision:** describe the dominant observed state and its reference viewport, then add a comment when a materially different composition is known. Do not add CSS-like breakpoint props.

## 2. Responsive geometry variants

The geometry grammar is a snapshot model. It does not yet state whether a node may carry alternate relational geometry at named viewport classes, nor how such classes remain independent of a framework's breakpoints.

**Open Question:** should a future variant model select whole spatial subtrees, override a small set of geometry props, or keep each viewport as a separate document?

## 3. Selection scope in master-detail UIs
...
## 5. Fixed versus data-driven options
...
## 6. Form fields and settings rows
...
## 8. Group headers and template scopes
...
## 9. Alternative and transient state
...
## 11. Disclosure and action ownership
...
## 13. Calendar, kanban, chart, and other complex primitives
...
## 15. Grid track grammar

Equal columns plus `columnSpan` and `rowSpan` cover many observed grids, while `Row` plus `flex` covers one-dimensional proportions. They do not compactly describe mixed fixed/flexible table or dashboard tracks.

**Open Question:** can a small relational track grammar express patterns such as fixed sidebar plus flexible content without admitting CSS grid-template strings?

## 16. Measurement tolerance

Reference-unit numbers improve reconstruction but independently extracted screenshots will produce slightly different measurements.

**Open Question:** should canonicalization define rounding bands or comparison tolerances, and should those vary by control, spacing, pane, and viewport geometry?

## 17. Anchored overlay fallback

`anchor` plus `placement` captures the semantic relationship of menus, popovers, and tooltips. Collision avoidance and fallback placement are not yet modeled.

**Open Question:** is an ordered semantic fallback list necessary, or should collision handling remain a renderer responsibility?
